...
FROM customers
WHERE country IN ('Spain', 'France', 'Italy');# 1370,1337,1401,1702
 SELECT EmployeeNumber,EmployeeNumber,lastname,email FROM  employees
 where EmployeeNumber in(1370,1337,1401,1702);
 # using join 
 select distinct(employeeNumber),firstname,email
 from  customers c
...
select * from customers;
select customerNumber,count(paymentDate) from payments 
where paymentDate<"2004-10-28"
group by customerNumber;

select c.customerName,p.customerNumber,count(p.paymentDate) from payments p
join customers c
on c.customerNumber= p.customerNumber
where paymentDate<"2004-10-28"
group by  c.customerNumber, p.customerNumber;

select c.customerName,p.customerNumber,count(p.paymentDate) as count_payments from payments p
join customers c
on c.customerNumber= p.customerNumber
where paymentDate<"2004-10-28"
group by  c.customerNumber, p.customerNumber
having count_payments>2;

#######################################
# Exercise : determine the total number of unit sold for each product
select * from products;
select * from orderdetails;
select productCode,sum(quantityOrdered) as unit_sold
from orderdetails group by productCode;

select o.productCode,productName,sum(quantityOrdered) as unit_sold
from orderdetails o
...
#############################################
# find product with second largest number of sold
select o.productCode,productName,sum(quantityOrdered) as unit_sold
from orderdetails o
inner join products p 
on o.productCode=p.productCode
group by o.productCode 
order by  unit_sold desc limit 1;
# find product with second largest number of sold ====>offset(rows skip)
select o.productCode,productName,sum(quantityOrdered) as unit_sold
from orderdetails o
inner join products p 
on o.productCode=p.productCode
group by o.productCode 
order by  unit_sold desc limit 1 offset 1;

select o.productCode,productName,sum(o.quantityOrdered) as unit_sold
from orderdetails o
inner join products p 
on o.productCode=p.productCode
group by o.productCode 
...
###############################################
# Q : display the list of the 5 most expensive products in the : motorcycles" 
# product line with their price (MSRP)rounder to dollars
select * from products;
select productName,buyPrice,round(MSRP) from products
where productLine="Motorcycles"
order by MSRP desc limit 5;
###############################
# Q. display  the product code, product name,buy price,sale price and profit margin
#percentage (((MSRP - buyPrice)*100/buyPrice),2) for the 10 products with the highest profit margin
# Round the profit margin to 2 decimals.
select * from products;
select productCode,productName,buyPrice,MSRP as sales_price, 
round(((MSRP - buyPrice)*100/buyPrice),2) as profit_margin from products
 order by  profit_margin  limit 10;
######################
#QUESTION: List the largest single payment done by every customer in the year 2004, 
#ordered by the transaction value (highest to lowest).
select customerNumber ,max(amount) as largestpayment from  payments
where year(paymentDate) =2004 group by  customerNumber order by  largestpayment  desc;
#QUESTION: Show the total payments received month by month for every year.
select year(paymentDate) as "year",month(paymentDate) as "month",round(sum(amount),2) as 
 "totalpayments" from  payments group by  "year","month" ;
#QUESTION: For the above query, format the amount properly with a dollar symbol 
#and comma separation (e.g $26,267.62), and also show the month as a string.
select year(paymentDate) as "year",format(paymentDate,"%b") as "monthname",
concat("$",format(sum(amount),2)) as "totalpayments" from  payments 
group by  "year",month(paymentDate),"monthname" order by "year",month (paymentDate);
# joins questions from classicmodels
# show the 10 most recent payments with customer details (name &phone no)
use classicmodels;
 select * from  payments;
select * from customers;
select c.customerNumber,amount,paymentDate,customerName,phone
from payments p 
inner join  customers c 
on p.customerNumber=c.customerNumber
order by paymentDate desc limit 10;
# show  the full office address and phone number for each employees
select * from offices;
select * from employees;
select addressLine1,addressLine2,phone,firstName,employeeNumber 
from offices o
...
select * from orderdetails od
 ...
 select * from employees;
 select e1.employeeNumber as emp_number,e1.firstName as emp_number,e2.employeeNumber as manager_number,
 e2.firstName as manager_name 
 from employees e1
 left join employees e2 
 on e1.reportsTo=e2.employeeNumber;
 # how many orders have been placed herkku gifts?
 select * from customers;
 select * from customers
 where  customerName="herkku gifts";
 select * from orders;# ordernumber,orderdate,requireddate,shippeddate,status,comments,customernumber
 select customerName,c.customerNumber,count(orderNumber) from orders o
 inner join customers c 
 on o.customerNumber=c.customerNumber
 where  customerName="Herkku Gifts"
 group by customerName,c.customerNumber;
 # classicmodels manufacturing company==>toys==>gifts shops,shops==>people,companies
#Q.3
...
# Q.4
-- delete the  entries for Sales Representative working in london
select * from employees; # secodery table
select * from offices; # primary table
select  officecode from  offices where city ="Lomdon"; # 7
select * from  employees where  officecode =7;
delete from  employees where employeeNumber in (1501,1504); # error 
delete from employees where  officecode=7;
 ...
 SELECT * FROM customers where customername like "%Toys%" ;
  ...
  ####################
  # classic_models_database
  -- Q. report the number of customer who have  made payments before october 28,2004
  use ClassicModels; 
  select * from customers;
  select * from payments;
  select count(*)from  payments; #273 ====>count of payments
...
WHERE paymentdate= '2004-10-28';
-- Q.6
select * from  payments where amount>10000 order by  amount desc;
...
select * from  customers;
select * from employees;
select c.customerName as account_representative
from customers c
...
-- Q.3 
select paymentDate, SUM(amount) as daily_total
from payments
...
-- Q.7
select e.firstName, e.lastName, e.jobTitle,city
from employees e
...
-- Q.9
select * from orderdetails;
select  SUM(od.quantityOrdered * od.priceEach)  as total_value_on_hold
from orderdetails od
join orders o on od.orderNumber = o.orderNumber
where o.status = "On Hold";
-- Q.10 
select c.customerName, COUNT(o.orderNumber) as on_hold_count
from customers c
join orders o ON c.customerNumber = o.customerNumber
where o.status = "On Hold"
group by c.customerName
order by on_hold_count desc;
-- Q.11
select o.orderDate, p.productName, p.productCode
from orders o
...
-- Q.17
select distinct.productName, DAYNAME(orderDate)
from products p
join orderdetails od on p.productCode = od.productCode
join orders o on od.orderNumber = o.orderNumber
